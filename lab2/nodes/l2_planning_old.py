...
class PathPlanner:
    ...
    # def circular_rollout(self, node_i, v, w, dt):
    #     x, y, theta = node_i.flatten()
    #     if w == 0:
    #         newx = x + v * np.cos(theta) * dt
    #         newy = y + v * np.sin(theta) * dt
    #     else:
    #         newx = x + v / w * (np.sin(theta + w * dt) - np.sin(theta))
    #         newy = y - v / w * (np.cos(theta + w * dt) - np.cos(theta))
    #     newt = theta + w * dt
    #     return np.array([newx, newy, newt])[:, np.newaxis]

    def robot_controller(self, node_i, point_s):
        #This controller determines the velocities that will nominally move the robot from node i to node s
        #Max velocities should be enforced
        # print("TO DO: Implement a control scheme to drive you towards the sampled point")
        _, angle = self.get_dist_angle_diff(node_i, point_s)

        v = self.vel_max * (1.0 - (min(abs(angle), np.pi / 2)) / np.pi * 2)# min(max(dist, -self.vel_max), self.vel_max)
        ...
        # v_values = np.arange(-self.vel_max, self.vel_max, 0.1)
        # w_values = np.arange(-self.rot_vel_max, self.rot_vel_max, 0.1)
        ...
        #         newq = self.trajectory_rollout(node_i, v, w)[:, -1:]
        #         cost = np.linalg.norm(newq[:2, :] - point_s, ord=1)
        #         if cost < min_cost:
    ...
    def points_to_robot_circle(self, points):
        ...
        for c in cells.T:
            ...
            all_pixels.extend(zip(rr, cc))

        pixels = np.unique(np.array(all_pixels), axis=0).T

        return pixels
    
    def check_trajectory_collision(self, points):
        pixels = self.points_to_robot_circle(points)
        if pixels.size == 0:
            return True
        rr, cc = pixels
